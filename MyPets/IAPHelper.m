...
-(NSString *)getExpiryDateString {
    if ([self daysRemainingOnSubscription] > 0) {
        ...
        [dateFormat setDateFormat:NSLS(@"dd/MM/yyyy")];
        return [NSString stringWithFormat:NSLS(@"Subscribed! \nExpires: %@ (%i Days)"),[dateFormat stringFromDate:today],[self daysRemainingOnSubscription]];
    } else {
        return NSLS(@"Not Subscribed");
    }
}
...
- (void)provideContentForProductIdentifier:(NSString *)productIdentifier {
    ...
    if ([productIdentifier hasSuffix:@"month"]) {
        if ([productIdentifier isEqualToString:@"br.com.morbix.mypets.plan_1month"]) {
            [self purchaseSubscriptionWithMonths:1];
        }else if ([productIdentifier isEqualToString:@"br.com.morbix.mypets.plan_3month"]) {
            [self purchaseSubscriptionWithMonths:3];
        }else if ([productIdentifier isEqualToString:@"br.com.morbix.mypets.plan_6month"]) {
            [self purchaseSubscriptionWithMonths:6];
        }else if ([productIdentifier isEqualToString:@"br.com.morbix.mypets.plan_12month"]) {
